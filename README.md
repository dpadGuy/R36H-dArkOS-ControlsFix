# R36H-ControlsFix-dArkOS
R36H-dArkOS-ControlsFix aims to bring the conventional R36H controls from ArkOS over to dArkOs. (This also means assigning the right hotkeys)

For this we use the R36H.sh script from DeviceType which originates from ArkOS in order to assign the right controls and hotkeys for the R36H.

This should in theory also work on other retro handheld consoles that do not have a FN key but i have not tried on them.

# Install Process
**Download the latest archive release which contains "DeviceType.zip", inside the archive you drag and drop the folder "DeviceType" inside "/roms/tools" or "EASYROMS/tools"**

**Over to your console you go "Main Menu -> Options -> Tools -> DeviceType"**

**You then run "R36H"**

**You will be prompted if you wanna set device to R36H or not**

**Enjoy ^_^**

# Emulator Menu Hotkeys:

**PPSSPP = R2 + L2**

**Drastic = L3**

**RetroArch = Select + X**

# Restoring To Default Controls
**"Main Menu -> Options -> Advanced"**

# Credits
https://github.com/christianhaitian/dArkOS

https://github.com/christianhaitian/arkos
